...
+Proof : (x : ℕ) → x + zero ≡ x
+Proof zero = refl
+Proof (succ x) = cong succ (+Proof x)

ex1 : ∀{A} l (xs : Vec A l) →  xs +++ [] ≡[ +Proof l ] xs 
ex1 zero [] = refl
ex1 (succ l) (x ∷ xs) = cong-cons x (+Proof l) (ex1 l xs)
...
module _ 
 ...
 where
  ...
  transport-iso : {a b : A} → (p : a ≡ b) →  transport p (transport (sym p) a) ≡ B b 
  transport-iso = ?
...
cong-cons2 : ∀{X m n} (x : X) {xs : Vec X m} {ys : Vec X n} (p : m ≡ n)
          → xs ≡'[ p ] ys → x ∷ xs  ≡'[ cong succ p ]  x ∷ ys
cong-cons2 _ refl refl = refl 
...
assoc+++ (succ l) m n (x ∷ xs) ys zs = goal
 where
  ...
  goal : x ∷ ((xs +++ ys) +++ zs)  ≡'[ cong succ (+-assoc l m n) ]
         x ∷ (xs +++ (ys +++ zs))
  goal = cong-cons2 x (+-assoc l m n) IH
