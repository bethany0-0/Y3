...
np.random.seed(0)
nb_of_training_samples = 20
nb_of_testing_samples = 100
X, t = datasets.make_moons(nb_of_training_samples, noise=0.15)
X_test, t_test = datasets.make_moons(nb_of_testing_samples, noise=0.15)
...
for k in range(K):
    ...
    training_t = [tt for i, tt in enumerate(t) if i % K != k]
    validation_x = [x for i, x in enumerate(X) if i % K == k]
    validation_t = [tt for i, tt in enumerate(t) if i % K == k]
...
## Set up drop out rate
## dropout_rate: the higher the probability, 
# the better chance to retain a neuron 
dropout_rate,do_dropout = (0.85,False)
...
nb_of_iterations = 2000  # Number of gradient descent updates
...
for epoch in range(nb_of_iterations):
    ...
    inx = [[i] for i in range(nb_of_training_samples)] 
    shuffle(inx)
    for d in range(nb_of_training_samples):
        ### The forward step ###:
        ...
        if(do_dropout):
            # Randomly set some of the layer's weight values to 0 
            # during forward propagation, which is done by 
            # binomial random variable.  np.random.binomial(N, p, size = q)
            # n = number of trials, in our case num_hidden_neurons
            # p = probability event of interest (retaining the hidden neurons) occurs on any one trial
            # p = dropout_rate, the higher the probability, the better chance to retain the neuron
            dropout_values = np.random.binomial([np.ones((1,num_hidden_neurons))],dropout_rate)[0]  
            a_layer1_p = np.multiply(a_layer1_p, dropout_values)
        ...
        # You can use matrix form to vectorise this double loop implementation       
...
y_hat = ann_predict(w1,w2,X_test)        
error = np.sum(np.abs(y_hat - t_test))
print(error)
plot_decision_boundary(w1, w2, X_test, t_test)
